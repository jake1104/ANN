# Change Log
# 2025-10-13: v05 - COMPLETE implementation with improvements
# - Enhanced backpropagation with chain rule
# - Using CrossEntropyLoss (already implemented)
# - Added model save/load functionality
# - Added proper batching support with DataLoader
...
class ModelSaver:
    ...
    def save_model(
        ...
        for name, param in model.named_parameters().items():
            save_dict[name] = cp.asnumpy(param.data)  # Convert to numpy

        # Add metadata if needed
        if epoch is not None:
            save_dict["_epoch"] = epoch  # Use underscore prefix for metadata

        # Create directory if it doesn't exist
        os.makedirs(os.path.dirname(filepath) if os.path.dirname(filepath) else ".", exist_ok=True)

        # Save to file using numpy (not cupy)
        import numpy as np
    ...
    def load_model(model: "TransformerArchitecture", filepath: str) -> Optional[int]:
        ...
        # Return epoch number if it was saved
        epoch = None
...
class AdamW:
    ...
    def step(self) -> None:
        ...
        for i, param in enumerate(self.params):
            ...
            param.data -= self.lr * (
                m_hat / (cp.sqrt(v_hat) + self.eps) + self.weight_decay * param.data
            )
...
class LayerNormalization:
    ...
    def backward(self, grad: cp.ndarray) -> cp.ndarray:
        ...
        # Gradient w.r.t input
        # Compute N but use it in calculation
        N = cp.prod(cp.array(grad.shape[:-1]))
        grad = grad / N  # Apply N to normalize gradients
        ...
        grad_mean = cp.sum(
            ...
            -2.0 * (self.x_normalized) * self.std, axis=-1, keepdims=True
        )
...
class MultiHeadAttention:
    ...
    def forward(
        ...
        self.q = (
            cp.matmul(q, self.wq.data)
            .reshape(batch_size, -1, self.num_heads, self.head_dim)
            .transpose(0, 2, 1, 3)
        )
        self.k = (
            cp.matmul(k, self.wk.data)
            .reshape(batch_size, -1, self.num_heads, self.head_dim)
            .transpose(0, 2, 1, 3)
        )
        self.v = (
            cp.matmul(v, self.wv.data)
            .reshape(batch_size, -1, self.num_heads, self.head_dim)
            .transpose(0, 2, 1, 3)
        )
    ...
    def backward(
        self, grad: cp.ndarray, training: bool = True
        ...
        grad_wq = cp.matmul(
            self.q_input.reshape(-1, self.d_model).T, grad_q.reshape(-1, self.d_model)
        )
        grad_wk = cp.matmul(
            self.k_input.reshape(-1, self.d_model).T, grad_k.reshape(-1, self.d_model)
        )
        grad_wv = cp.matmul(
            self.v_input.reshape(-1, self.d_model).T, grad_v.reshape(-1, self.d_model)
        )
...
class EncoderLayer:
    ...
    def forward(
        ...
        self.attn_output, _ = self.mha.forward(
            self.norm_x, self.norm_x, self.norm_x, mask, training
        )
...
class DecoderLayer:
    ...
    def backward(
        ...
        grad_norm_out1, grad_enc_k, grad_enc_v = self.mha2.backward(
            grad_attn2, training
        )
...
class Encoder:
    ...
    def backward(self, grad: cp.ndarray, training: bool = True) -> None:
        ...
        _, _ = self.input_ids.shape  # Dimensions used implicitly in embedding layer
...
class TransformerArchitecture:
    ...
    def create_look_ahead_mask(size: int) -> cp.ndarray:
        """
        Create a look-ahead mask for the decoder's self-attention.
        This mask ensures that during training, the decoder can only attend to previous positions.
        """
        mask = 1 - cp.triu(cp.ones((size, size)), k=1)
    ...
    def named_parameters(self) -> Dict[str, Parameter]:
        ...
        for i, param in enumerate(self.encoder.parameters()):
            if hasattr(param, "name"):
                named_params[f"encoder.{param.name}"] = param
            else:
                named_params[f"encoder.param_{i}"] = param

        # Decoder parameters
        for i, param in enumerate(self.decoder.parameters()):
            if hasattr(param, "name"):
                named_params[f"decoder.{param.name}"] = param
            else:
                named_params[f"decoder.param_{i}"] = param
    ...
    def backward(
        ...
        Args:
            ...
            training: Whether in training mode
            accumulate: Whether to accumulate gradients (True) or zero them first (False)
        """
...
def sparse_categorical_crossentropy(
    ...
    """
    batch_size, seq_len, _ = logits.shape  # Shape dimensions used for indexing
