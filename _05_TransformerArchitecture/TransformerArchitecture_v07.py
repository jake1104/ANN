# Change Log
# 2025-10-18: v07 - Performance and Memory Optimizations
# - Mask creation: Vectorized to avoid repeated loops
# - Mixed Precision: Properly applied to forward computations
# - Gradient zeroing: Added set_to_none option for memory efficiency
# - DataLoader: Optimized shuffle using NumPy permutation
# - QKV Fusion: Properly implemented with single matmul
# - Memory Pool: CuPy allocator optimization
# - Gradient Checkpointing: Conditional based on config
# - Hyperparameters: Adjusted warmup_ratio (0.05), label_smoothing (0.05)
# - Sequence length: Reduced default max_seq_len to 60
...
# Set CuPy memory pool for optimization
...
class DataLoader:
    """Data loader with optimized shuffling."""
...
class Tensor:
    ...
    def zero_grad(self, set_to_none: bool = True) -> None:
        """Reset gradients to None or zeros."""
        if set_to_none:
            self.grad = None
        else:
...
class MultiHeadAttention:
    """Multi-Head Attention with proper QKV fusion."""
    ...
    def __init__(self, d_model: int, num_heads: int, dropout_rate: float = 0.1):
        ...
        # Fused QKV projection - proper initialization
        ...
        self.enc_output_for_cross: Optional[cp.ndarray] = None
    ...
    def forward(
        ...
        # Scaled dot-product attention in FP16 if enabled
        if GlobalConfig.USE_MIXED_PRECISION:
            q_compute = self.q.astype(cp.float16)
            k_compute = self.k.astype(cp.float16)
            scores = cp.matmul(q_compute, k_compute.transpose(0, 1, 3, 2)).astype(cp.float32) * self.scale
        else:
        ...
        if GlobalConfig.USE_MIXED_PRECISION:
            attn_compute = self.attn_weights.astype(cp.float16)
            v_compute = self.v.astype(cp.float16)
            self.attn_output = cp.matmul(attn_compute, v_compute).astype(cp.float32)
        else:
    ...
    def backward(self, grad: cp.ndarray, training: bool = True) -> Tuple[cp.ndarray, Optional[cp.ndarray]]:
        ...
        if seq_len_q == seq_len_k:
            # Self-attention: stack gradients
...
class DecoderLayer:
    ...
    def _cross_attention(
        ...
        """Cross-attention helper with mixed precision."""
        ...
        self.mha2.k = K
        self.mha2.v = V
        self.mha2.qkv_input = q
        
        # Scaled dot-product attention with mixed precision
        if GlobalConfig.USE_MIXED_PRECISION:
            Q_compute = Q.astype(cp.float16)
            K_compute = K.astype(cp.float16)
            scores = cp.matmul(Q_compute, K_compute.transpose(0, 1, 3, 2)).astype(cp.float32) * self.mha2.scale
        else:
        ...
        # Apply to values
        if GlobalConfig.USE_MIXED_PRECISION:
            attn_compute = attn_weights.astype(cp.float16)
            V_compute = V.astype(cp.float16)
            attn_output = cp.matmul(attn_compute, V_compute).astype(cp.float32)
        else:
...
class TransformerArchitecture:
    """Complete Transformer with v07 optimizations."""
    ...
    def forward(
        ...
        """Forward pass with proper mixed precision."""
        # Encoder
        ...
        # Final projection with mixed precision
        if GlobalConfig.USE_MIXED_PRECISION:
...
def create_combined_mask(tar: cp.ndarray, pad_token: int = None) -> cp.ndarray:
    """Create combined look-ahead and padding mask (vectorized)."""
    if pad_token is None:
        pad_token = GlobalConfig.PAD_TOKEN
    ...
    # Look-ahead mask
    look_ahead = create_look_ahead_mask(seq_len)  # (1, 1, seq_len, seq_len)
    
    # Padding mask
    padding = create_padding_mask(tar, pad_token)  # (batch, 1, 1, seq_len)
    
    # Vectorized combination: broadcast and multiply
    # look_ahead: (1, 1, seq_len, seq_len) -> need to mask columns
    # padding: (batch, 1, 1, seq_len) -> expand to match
    
    # Expand padding to match look_ahead dimensions
    padding_expanded = padding.repeat(seq_len, axis=2)  # (batch, 1, seq_len, seq_len)
    
    # Combine: element-wise minimum
    combined = cp.minimum(look_ahead, padding_expanded)
...
class Trainer:
    ...
    def validate(
        ...
        loss, _ = cross_entropy_with_label_smoothing(
            logits, tar_real, label_smoothing=0.0
        )
        acc = compute_accuracy(logits, tar_real)
...
if __name__ == "__main__":
    print("\n" + "=" * 80)
    print("Transformer v07 - Optimized Training")
    print("=" * 80)
    ...
    print("\nComputing loss with label smoothing (0.05)...")
    loss, grad = cross_entropy_with_label_smoothing(
    ...
    print("\n" + "=" * 80)
    print("✓ v07 test complete!")
    print("Key improvements:")
    print("  ✓ Vectorized mask creation")
    print("  ✓ Proper mixed precision (FP16)")
    print("  ✓ Optimized gradient zeroing (set_to_none)")
    print("  ✓ NumPy-based shuffle")
    print("  ✓ QKV fusion")
    print("  ✓ CuPy memory pool")
    print("=" * 80)
