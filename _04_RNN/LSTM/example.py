...
from .LSTM_v05 import LSTM, create_batches_for_embedding
import os
import numpy as np
import cupy as cp
...
usage_message = '''
================================================================================
LSTM v5 (Adam Optimized) 사용법
================================================================================
- 스크립트를 그냥 실행하면, 기존에 학습된 'lstm_model.npz'를 불러와 이어서 학습합니다.
...
- 예시:
  ...
  python LSTM_v05.py --epochs 500 --plot

  # 특정 경로의 모델을 불러와 학습 후 다른 경로에 저장
  python LSTM_v05.py --load-path my_lstm.npz --save-path new_lstm.npz

  # 모델을 무시하고 처음부터 1000 에포크 재학습
  python LSTM_v05.py --force-train --epochs 1000
================================================================================
'''
print(usage_message)

parser = argparse.ArgumentParser(description="Train and evaluate the LSTM v5 with Adam.")
...
model = LSTM(vocab_size=vocab_size, embedding_dim=embedding_dim, n_h=n_h, n_y=vocab_size)
...
if args.plot and loss_history:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Training Loss History (LSTM v5 Adam)")
    plt.grid(True)
    plt.show()
