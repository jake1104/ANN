...
# Change Log
# 2025-10-09: @cupy.fuse()을 이용한 커널 퓨전 최적화

import cupy as cp
import numpy as np

class LSTM:
  # --- Fused Kernel Definitions ---
  # @cp.fuse() 데코레이터는 여러 원소별 연산을 하나의 GPU 커널로 통합하여
  # 커널 호출 오버헤드를 줄이고 성능을 향상시킵니다.
  @staticmethod
  @cp.fuse()
  ...
  def forward(self, X_batch_idx):
    ...
    for t in range(seq_len):
      ...
      # Call the fused kernel for cell computation
      h_t, c_t, i_t, f_t, o_t, g_t = self._fused_forward_cell(
          gates[:, :self.n_h], gates[:, self.n_h:2*self.n_h], 
          gates[:, 2*self.n_h:3*self.n_h], gates[:, 3*self.n_h:], c_prev)
  ...
  def backward(self, Y_batch, ys, caches):
    ...
    for t in reversed(range(seq_len)):
      ...
      # Call the fused kernel for gradient computation
      dgate_i, dgate_f, dgate_o, dgate_g, dc_next = self._fused_backward_cell(
          dh, dc_next, c_prev, c_t, i_t, f_t, o_t, g_t)
  ...
  def predict(self, seed_sequence, item_to_idx, idx_to_item, max_len, end_idx):
    ...
    for item in seed_sequence:
      ...
      h, c, _, _, _, _ = self._fused_forward_cell(
          gates[:, :self.n_h], gates[:, self.n_h:2*self.n_h], 
          gates[:, 2*self.n_h:3*self.n_h], gates[:, 3*self.n_h:], c)
